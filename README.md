...
```bash
python smoke_test.py   # 전체 기능
python fix_test.py     # 가산 9시간·휴게 정책·기록 추가/삭제·시간대
```
...
- **가산수당**: 하루 **9시간 초과분**에 대해 50% 가산 (직원별 '가산' 체크가 켜진 경우만)
- **휴게시간**: 직원별 기본값 3가지 — 퇴근 시 물어봄(권장) / 법정 자동차감 / 차감 안 함
  - '차감 안 함' 직원은 퇴근 버튼을 누르면 질문 없이 바로 처리됩니다
  - '물어봄' 직원은 안 쉼 / 30분 / 1시간 중 선택, 1시간 이상이면 30분 단위로 조정
- **주휴수당**: 수동 지정 → 직원별 정책 → 법정 자동 판정 순
- **시간대**: 모든 기록은 한국 시간(Asia/Seoul) 기준으로 저장됩니다
